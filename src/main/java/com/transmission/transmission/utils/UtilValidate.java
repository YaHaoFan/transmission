package com.transmission.transmission.utils;
...
//    }
     ...
     * 判断对象的属性属性哪些为空
     * @param obj
     * @return List存储的为字段值为空的字段 ,size ==0 为该对象所有属性不为空, 
     */
    public List<String> checkObjFieldIsNull(Object obj) {
    	List<String> list=new ArrayList<String>();
        for(Field f : obj.getClass().getDeclaredFields()){
            f.setAccessible(true);
            try {
				if(f.get(obj) == null ||f.get(obj).toString().equals("")){
					list.add(f.getName());
				}
